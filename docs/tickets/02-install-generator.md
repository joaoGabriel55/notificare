...
## Scope
- `lib/generators/active_job/notificare/install/install_generator.rb` (Rails generator).
...
- Initializer `config/initializers/active_job_notificare.rb` with documented defaults.
- Commented `mount ActiveJob::Notificare::Engine => "/notificare"` appended to `config/routes.rb`.
- Shared view partials seeded under `app/views/active_job/notificare/` (empty stubs; ticket 09 fills them).
...
## Acceptance criteria
- `rails generate active_job:notificare:install` produces the migration, initializer, route stub, and partial files.
